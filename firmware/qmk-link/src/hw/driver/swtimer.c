...
static repeating_timer_t swtimer_hw_timer;


static bool swtimerHwCallback(repeating_timer_t *t)
{
  (void)t;
  swtimerISR();
  return true;
...
bool swtimerInit(void)
{
  ...
  // 1ms 주기 틱. 음수는 "직전 콜백 시작 기준" 이라 지터가 누적되지 않는다.
  add_repeating_timer_us(-1000, swtimerHwCallback, NULL, &swtimer_hw_timer);
