...
### 8. PC 로 보내는 키 리포트는 버리지 않는다

`tud_hid_n_ready()` 가 false 면 예전에는 그냥 버렸다. **QMK 는 재시도하지 않으므로
그 키 이벤트는 영영 사라진다.** 눌림이 유실되면 안 눌린 것이 되고, 뗌이 유실되면
PC 에 키가 눌린 채로 남는다.

엔드포인트가 바쁜 건 예외 상황이 아니다 — 실측에서 **보내기 1138회에 not ready 1791회**다.

키보드 리포트는 "지금 눌린 상태" 라 최신 것만 있으면 된다. 큐가 아니라 보류 한 칸을
두고 `usbdHidUpdate()` 가 계속 흘려보낸다. 실측 **23회**가 이 재시도로 나갔다.

★ 섀도는 **실제로 나간 뒤에** 갱신한다. 먼저 갱신하면 실패했을 때 "이미 보냈다" 고
기억해 같은 리포트를 영영 다시 안 보낸다.
...
| 7 | Test Matrix | ✅ `VIA_INSECURE`. 눌린 usage 가 배열에서 반짝인다 |
| 8 | 키 유실 없음 | ✅ 실사용 `호출 1154 / 보냄 1138 / 나중에보냄 23 / 보류 0`, `전송실패 0` |

## 진단 도구 — `key info`

이 단계에서 겪은 고장들은 **`mounted` / `drop` 으로는 안 보였다.** 그래서 만들었다.

```
drain        : 1737        큐에서 꺼낸 리포트
  -> link    : 1737        keyboard 프로토콜 + len>=8 (정상 경로)
  -> 버림    : 0           그 외 — 조용히 사라지는 것
PC 로 보내기 (IF0 키보드)
  호출 1154  보냄 1138  같아서 건너뜀 0
  not ready 1791   전송실패 0   나중에보냄 23   보류 0
  ready 1  mounted 1  suspend 0
```

| 이 값이 이상하면 | 무엇을 의심하나 |
|---|---|
| `버림` 이 증가 | USB 호스트가 길이 0 리포트를 뱉는다 → 플래시 정지로 장치가 서스펜드 |
| `link` 이 `drain` 보다 훨씬 적음 | 위와 같다 |
| **`보류` 가 1 에서 안 내려감** | 못 보낸 리포트가 갇혔다. PC 는 키가 눌린 채로 안다 |
| `나중에보냄` 이 0 인데 `not ready` 가 큼 | 재시도 루프가 안 돌고 있다 (`cliLoopIdle()` 확인) |
| `전송실패` 가 증가 | `tud_hid_n_report()` 자체가 실패 |
