...
#### Vial 원본 키보드는 거의 다 된다 — 우리가 특이한 것이다

vial-qmk 의 `vial.json` 을 가진 키보드 **635개를 세어 봤다.**
...
못 되는 넷은 `bpiphany`(18x8), `era`(10x16) x2, **`converter/adb_usb`(16x8)** 다.
전부 **행이 많은** 보드다. 넷째가 ADB->USB 변환기라는 게 말해 준다 —
**같은 부류가 같은 벽에 부딪힌다.**

흔한 모양은 5x15(10 B) · 6x16(18 B) 다. **풀사이즈 96키도 18 B 로 넉넉하다.**
문제는 키 개수가 아니라 **행 수**다 — 식이 `(cols/8+1) x rows` 라 행에 비례한다.
우리는 좌표가 usage 라 `row = usage>>4` 로 **16행**이 된다.

**접어도 안 된다.** 256키는 최소 32 B 인데 이 형식의 여유가 28 B 다:

```
 16행 x  16열 = 256키 -> 48 B      2행 x 128열 = 256키 -> 34 B
  8행 x  32열 = 256키 -> 40 B      1행 x 256열 = 256키 -> 33 B
  4행 x  64열 = 256키 -> 36 B
```

8x16(128키)이면 24 B 로 들어가지만 usage `0x80` 이상이 잘린다 —
**모디파이어 `0xE0`~`0xE7` 이 거기 있어서** 버릴 수 없다.

#### 매트릭스를 줄여 살리는 안 — 검토했고 하지 않기로 했다

**행↔열 재배치만으로는 안 된다.** 256키 = 32 B 인데 여유가 28 B 라 어떤 모양도 넘친다:

```
16x16 -> 48 B    8x32 -> 40 B    4x64 -> 36 B    2x128 -> 34 B    1x256 -> 33 B
```

**안 쓰는 usage 를 빼면 된다.** 실물 키보드가 보내는 것은
`0x00~0xA4` + `0xE0~0xE7` = **173개**뿐이다:

| 모양 | 칸 | 필요 | |
|---|---:|---:|---|
| 12행 x 15열 | 180 | 24 B | 여유 있음 |
| 14행 x 15열 | 210 | 28 B | 한계선 |

버리는 것은 `0xA5~0xDF`(예약 + `KP_XOR`·천단위구분자 같은 키패드 확장)와
`0xE8~0xFF`(예약)로, 실물이 보내지 않는 영역이다.

**그런데 하지 않는다.** 대가가 얻는 것보다 크다:

| 얻는 것 | 잃는 것 |
|---|---|
| Vial 앱 매트릭스 테스터 | **좌표 = usage 성질** — 이 프로젝트의 모든 문서와 도구(`gen_keymap.py` · `qmk matrix` · 09단계 마법사)가 그 위에 서 있다 |
| 동적 키맵 4096 -> 2880 B | `link.c` · `matrix.c` · `keymap.c` · `config.h` · `gen_keymap.py` · CLI 에 변환표 (두 트리 모두) |
| | 저장된 키맵이 전부 깨진다 |

대체 수단이 이미 있다 — CLI `qmk matrix`, 그리고 09단계의 학습 마법사가
더 나은 도구가 된다 (자리 강조 · 드래그앤드롭 · 저장).
