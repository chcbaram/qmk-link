...
7. **저장 SLOT** 을 고르고 **[이 SLOT 에 담기]** — 보드에 저장된다
8. 파일이 필요하면 내려받는다 (`layout-via.json` / `layout-vial.json` / `layout-kle.json`)
...
`tools/kbd_upload.py` 는 파일을 그대로 담거나 꺼낼 때 쓰는 도구로 남아 있다.
...
| `app.js` | WebHID · KLE 파싱 · 마법사 · 내보내기 · SLOT 담기/지우기 |
