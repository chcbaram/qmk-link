...
1. ② 트리의 **[새 SLOT 추가]** — 빈 자리가 잡힌다 (아직 보드는 안 건드린다)
2. ③ 에서 배열을 넣고 키를 배운다
3. **[SLOT n 에 담기]**
...
**PID 가 바뀔 때만**이다. 그러니까 *적용되는 SLOT 이 바뀔 때*다.

| | 끊기나 |
|---|---|
| 같은 SLOT 을 고쳐 담기 | 안 끊긴다 |
| 쓰던 것이 있는데 새 SLOT 에 담기 | 안 끊긴다 (적용은 그대로) |
| 첫 SLOT 을 담기 · **[적용]** · 적용 중인 SLOT 지우기 | 끊긴다 |

끊길지 말지는 **보드가 응답에 실어 준다.** 같은 규칙을 웹에 한 벌 더 적으면
반드시 갈라지기 때문이다.

끊기면 브라우저는 **PID 가 다르면 다른 장치**로 본다. 전에 허락한 적 있는
